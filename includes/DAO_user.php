...
class DTO_pengguna
{
    ...
    {
        $this->idUser = $data['id_pengguna'];
        $this->pass = null;
        $this->role = $data['role'];
    }

    function setNewPass(?string $newPass = null)
    {
        $this->pass = $newPass;
    }
...
class DAO_pengguna
{
    ...
    //buat endpoint untuk notification
    static function getEmailById($idUser): ?string
    {
        $conn = Database::getConnection();
        $sql = "select email from m_pengguna where id_pengguna = ?";
        try {
            $stmt = $conn->prepare($sql);
            $stmt->execute([$idUser]);
            $hasil = $stmt->fetchColumn(); // Mengambil satu kolom saja

            return $hasil ?: null;
        } catch (PDOException $e) {
            custom_log("DAO_user::getEmailById : " . $e->getMessage(), LOG_TYPE::ERROR);
            return null;
        }
    }
