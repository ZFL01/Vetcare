    ...
    <div class="container mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex items-center justify-between h-20">
            <div class="flex items-center gap-3 group cursor-pointer" onclick="navigateTo('?route=')">
                <div
                    class="w-10 h-10 md:w-12 md:h-12 bg-gradient-to-br from-purple-500 to-purple-600 rounded-xl flex items-center justify-center group-hover:scale-110 transition-transform duration-300 shadow-glow">
                    <span class="text-white text-xl md:text-2xl">🐾</span>
                </div>
                <span
                    class="text-2xl md:text-3xl font-display font-bold text-purple-600 hover:text-purple-700 transition-colors">VetCare</span>
            </div>

            <nav class="hidden md:flex items-center gap-6 lg:gap-10">
                <button onclick="scrollToSection('dokter')"
                ...
                <div class="relative" id="services-container">
                    <button
                        class="relative text-gray-700 hover:text-purple-600 font-medium transition-all duration-300 group flex items-center gap-1">
                        <span>Layanan</span>
                        <svg class="w-4 h-4 transition-transform duration-300" id="services-chevron" fill="none"
                    ...
                    <div class="absolute top-full left-0 mt-2 w-64 bg-white backdrop-blur-xl border border-gray-200 rounded-xl shadow-hero py-3 z-[100] hidden opacity-0 transform scale-95 transition-all duration-300"
                        id="services-menu">
            ...
            <div class="flex items-center gap-3 md:gap-4">
                    ...
                    <div class="relative hidden md:block" id="user-menu-container">
                        <button
                            ...
                            <?php if ($isDokter): ?>
                                ...
                                <?php else: ?>
                                    <img src="<?php echo URL_FOTO . 'dokter-profil/default-profile.webp'; ?>" alt="Profile"
                                        class="w-8 h-8 rounded-full object-cover border border-purple-200">
                                <?php endif; ?>
                            <?php else: ?>
                                <div
                            ...
                            <svg class="w-4 h-4 text-gray-400 group-hover:text-purple-600 transition-colors" fill="none"
                                stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7">
                                </path>
                            </svg>
                ...
                <?php else: ?>
                    <button onclick="navigateTo('?route=auth')"
                        class="hidden md:block hover:bg-purple-50 font-medium px-4 py-2 rounded-lg transition-colors">
                        <span class="mr-2">👤</span>
                        Masuk
                    </button>
                    <button onclick="navigateTo('?route=auth')"
                        class="hidden sm:flex font-display font-semibold bg-gradient-to-r from-purple-500 to-purple-600 text-white px-6 py-3 rounded-2xl hover:from-purple-600 hover:to-purple-700 transition-all duration-300 shadow-glow">
                        <span class="mr-2">✨</span>
                        Daftar Sekarang
                    </button>
                <?php endif; ?>
                ...
                <!-- Mobile Menu Button -->
                <button class="md:hidden p-2 text-gray-700 hover:text-purple-600 transition-colors"
        ...
        <div class="md:hidden border-t border-purple-200 py-4 hidden" id="mobile-menu">
            <nav class="flex flex-col space-y-4">
                <button onclick="scrollToSection('dokter'); toggleMobileMenu()"
                    class="text-left text-gray-700 hover:text-purple-600 font-medium transition-colors px-4">
                    Dokter
                </button>

                <div class="px-4">
                    <button onclick="toggleMobileSubmenu('mobile-layanan')"
                        class="flex items-center justify-between w-full font-medium text-gray-700 hover:text-purple-600 transition-colors">
                        Layanan
                        <svg id="arrow-mobile-layanan" class="w-4 h-4 transition-transform duration-200" fill="none"
                            stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7">
                            </path>
                        </svg>
                    ...
                    <div id="mobile-layanan" class="hidden mt-2 space-y-2 pl-4 border-l-2 border-purple-100 ml-1">
                        ...
                        <a href="?route=tanya-dokter" onclick="toggleMobileMenu()"
                            class="block text-left text-sm text-gray-600 hover:text-purple-600 transition-colors">❓
                            Tanya Dokter</a>
                        <a href="?route=klinik-terdekat" onclick="toggleMobileMenu()"
                            class="block text-left text-sm text-gray-600 hover:text-purple-600 transition-colors">📍
                            Klinik Terdekat</a>
                        <a href="?route=dokter-ternak" onclick="toggleMobileMenu()"
                            class="block text-left text-sm text-gray-600 hover:text-purple-600 transition-colors">🐄
                            Dokter Ternak</a>
                        <a href="?route=dokter-hewan-kecil" onclick="toggleMobileMenu()"
                            class="block text-left text-sm text-gray-600 hover:text-purple-600 transition-colors">🐱
                            Dokter Hewan Kecil</a>
                    </div>
                </div>
                ...
                <div class="px-4 pt-4 border-t border-purple-200 space-y-3">
                    <?php if (isset($_SESSION['user'])) {
                        ...
                        <div class="flex items-center gap-3 mb-4">
                            <?php if ($isDokter): ?>
                                <?php if ($fotoProfile): ?>
                                    <img src="<?php echo URL_FOTO . 'dokter-profil/' . $fotoProfile; ?>" alt="Profile"
                                        class="w-10 h-10 rounded-full object-cover border border-purple-200">
                                <?php else: ?>
                                    <img src="<?php echo URL_FOTO . 'dokter-profil/default-profile.webp'; ?>" alt="Profile"
                                        class="w-10 h-10 rounded-full object-cover border border-purple-200">
                                <?php endif; ?>
                            <?php else: ?>
                                <div
                                    class="w-10 h-10 bg-purple-100 rounded-full flex items-center justify-center text-purple-600 font-bold text-lg">
                    ...
                    <?php } else { ?>
                        <button id='auth' onclick="navigateTo('?route=auth'); toggleMobileMenu()"
                            class="w-full justify-start hover:bg-purple-50 font-medium px-4 py-2 rounded-lg transition-colors flex items-center">
                            <span class="mr-2">👤</span>
                            Masuk
                        </button>
                        <button id="auth" onclick="navigateTo('?route=auth&action=register'); toggleMobileMenu()"
                            class="w-full font-display font-semibold bg-gradient-to-r from-purple-500 to-purple-600 text-white px-6 py-3 rounded-2xl hover:from-purple-600 hover:to-purple-700 transition-all duration-300 shadow-glow">
                            <span class="mr-2">✨</span>
                            Daftar Sekarang
                        </button>
                    <?php } ?>
