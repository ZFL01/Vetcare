...
<div id="konsultasiModal" class="fixed inset-0 z-50 hidden overflow-y-auto">
  <div class="flex items-center justify-center min-h-screen px-4">
    ...
    <!-- Modal Panel -->
    <div class="inline-block align-middle bg-white rounded-2xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:max-w-2xl sm:w-full" style="max-width:600px !important;">
      <div class="bg-white px-6 py-4" style="max-height:85vh; overflow:auto;">
        ...
        <div class="mb-6 p-4 bg-blue-50 rounded-lg border border-blue-200">
          <p class="text-sm text-blue-800">
            <span class="font-medium">ℹ️</span> Informasi yang Anda berikan akan membantu dokter hewan memberikan konsultasi yang lebih akurat dan tepat.
          </p>
        </div>
        ...
        <form id="konsultasiForm" onsubmit="submitKonsultasi(event)" class="space-y-6">
          ...
          <div>
            ...
            <input 
              type="text" 
              name="nama_hewan" 
              placeholder="Contoh: Kitty, Max, Bella" 
          ...
          <div>
            ...
            <input 
              type="text" 
              name="usia_hewan" 
              placeholder="Contoh: 2 tahun, 6 bulan, 3 minggu" 
              class="w-full px-4 py-3 bg-gray-100 rounded-lg border border-gray-200 focus:border-purple-500 focus:outline-none focus:ring-2 focus:ring-purple-200 transition-all"
              required>
          </div>
          ...
          <div class="flex gap-3 pt-4 border-t border-gray-200">
            <button type="button" onclick="closeKonsultasiModal()" class="flex-1 px-4 py-3 text-base bg-gray-200 text-gray-800 rounded-lg hover:bg-gray-300 transition-colors font-semibold">
              Kembali
            </button>
            <button type="submit" class="flex-1 px-4 py-3 text-base bg-gradient-to-r from-purple-500 to-purple-600 text-white rounded-lg hover:from-purple-600 hover:to-purple-700 transition-all font-semibold shadow-md">
        ...
        <div class="mt-8 pt-6 border-t border-gray-200">
          ...
          <div class="space-y-3">
            <div class="flex items-start p-3 bg-gray-50 rounded-lg border border-gray-200 hover:border-purple-300 hover:bg-purple-50 transition-colors cursor-pointer">
              <div class="flex-1">
...
function submitKonsultasi(event) {
  ...
  console.log('[KONSULTASI] Data submitted:', data);
  
  // Store in sessionStorage untuk digunakan di halaman chat-dokter
  sessionStorage.setItem('konsultasiData', JSON.stringify(data));
  
  // Close modal
  closeKonsultasiModal();
  
  // Redirect ke halaman chat-dokter dengan dokter ID
  if (window.currentDokterId) {
    console.log('[KONSULTASI] Redirecting to chat with dokter_id:', window.currentDokterId);
    window.location.href = '?route=chat&dokter_id=' + window.currentDokterId;
  } else {
